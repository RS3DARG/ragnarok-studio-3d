...
export default function ComingSoon({ items }: { items: UpcomingFigure[] }) {
  if (!items.length) return null;

  return (
    <section id="proximamente" className="mx-auto max-w-7xl px-4 py-16 sm:px-6">
      <div className="mb-8">
        ...
        {items.map((item) => (
          ...
          >
            ...
            <div className="p-4">
              ...
              {item.saga ? (
                <p className="mt-0.5 text-sm text-ember-400/90">{item.saga}</p>
              ) : null}
              {item.figure_type ? (
                <p className="mt-2 text-xs text-zinc-500">{item.figure_type}</p>
              ) : null}
            </div>
          </article>
        ))}
